            ...
            foreach ($subMenus as $s => $subMenu) {
                ...
                if (empty($subMenu["url"])) {
                    ...
                    if ($subMenu["id"] == "487" && $globalZone != "admin") //Calendars submenu under School Operations
                    {
                        ...
                        if($ySubeKodu != "0" || $ySubeKodu != $this->hqID)
                        {
                            $dbi->where("schoolId", array($ySubeKodu, $this->hqID, 0), "IN");    
                        }
                        else $dbi->where("schoolId", $ySubeKodu);
                        $dbi->where("active", "on");
                        $dbi->orderBy("calendarOrder", "asc");
                        $activeCalendars = $dbi->get(_CALENDARS_);
                        foreach ($activeCalendars as $activeCalendar) {
                            
                            if($activeCalendar["schoolId"] == "0" || $activeCalendar["schoolId"] == $this->hqID)
                            {
                                if($activeCalendar["allSchoolsCanSee"] == "off") continue;
                            }
